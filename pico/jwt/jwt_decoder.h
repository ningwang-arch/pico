...
public:
    ...
    explicit JWTDecoder(const std::string& jwt);
    ...
    std::vector<T> getHeaderValueAsArray(const std::string& key,
        ...
        if (m_header.isMember(key)) {
            std::vector<T> ret;
            for (auto& i : m_header[key]) {
                ret.push_back(i.as<T>());
            }
            return ret;
        }
        return default_value;
